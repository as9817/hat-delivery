...
## 2026-07-13 — Kakao 키워드검색 채점식 개선 배포 검증

- **대상**: `functions/lib/receipt-utils.js` — `kakaoKeywordSearch()`의 채점 로직을 `scoreKakaoKeywordCandidate(query, doc, distKm)` 순수함수로 분리, 카테고리 가점(+30, "주거시설"/"공동주택")·감점(-50, "분양사무소"/"분양"/"중개업"/"입출구") 추가, 거리 페널티 가중치 1.0→0.3 완화
- **커밋**: `a3bcaee`
- **배경(원인 재확인)**: 실제 라이브 오배송 사고 발생 — 아파트 단지명으로 OCR 인식됐는데 그 단지의 분양홍보관(부동산 판촉시설) 주소로 잘못 표준화됨. 긴급 read-only 분석 라운드에서 실제 라이브 Kakao 키워드검색 API를 서버와 동일한 파라미터(테넌트의 공개 마트 좌표/반경)로 직접 호출해 원인을 확정: 5개 후보 중 진짜 아파트는 1개뿐이었고, 분양홍보관이 그 아파트보다 약 1.4km 마트에 더 가깝다는 이유만으로 기존 채점식(`matchCount*100 - 거리(km)`)에서 근소한 점수 차로 1등이 됐음. API 응답에 이미 있던 `category_name` 필드(부동산>주거시설>아파트 vs 부동산>부동산서비스>분양사무소 등)를 전혀 활용하지 않고 있었던 것이 핵심 원인.
- **배포 전 확인**: `git status` clean(2개 파일만 스코프: `functions/lib/receipt-utils.js`, `functions/test/receipt-utils.kakao.test.js`), `node --test "test/*.test.js"` **133/133 pass**(기존 127 + 신규 6 — 실사고 당시 실측한 카카오 응답 5건을 그대로 mock 데이터로 사용한 회귀 재현 테스트 포함: 수정 전 채점식이면 여전히 오답이 선택됨을 먼저 확인한 뒤, 수정된 채점식으로는 정답 후보가 선택됨을 lat/lng까지 확인).
- **배포**: `firebase deploy --only functions --project hatdelivery-saas`만 실행. Hosting/`database.rules.json`/`storage.rules` 배포 안 함.
- **배포 후 라이브 검증(2단계)**:
  1. **1단계 — 지리적 조건 재현**: testmart 계정 세션으로 `geocodeAddress` 엔드포인트를 호출하되, 실사고 테넌트의 공개 마트 좌표(사업장 소재지, 개인정보 아님)를 직접 파라미터로 지정해 동일한 검색 조건을 재현 — 응답 `road_address`가 정답 아파트의 도로명주소로, `lat`/`lng`도 그 아파트 좌표와 정확히 일치함을 확인(수정 전에는 분양홍보관 주소가 나왔던 것과 대비).
  2. **2단계 — 실제 사고 테넌트/실제 계정으로 전체 파이프라인 재현**: 사고가 발생한 테넌트의 테스트용 기사 계정(사용자가 채팅으로 직접 전달, 비밀번호는 본 로그에 기록하지 않음)으로 실제 로그인 후, 캔버스 렌더링 합성 영수증(가짜 이름/가짜 전화번호, "건물명 + 동/호수" 패턴 — 실제 사고와 동일한 주소 형식이나 동/호수는 임의값)을 실제 `processReceipt` 엔드포인트로 호출 — **road_address가 정답 아파트 도로명으로 정확히 선택됨을 확인**, `lat`/`lng`도 아파트 좌표와 일치, `detail_address`도 "동/호수 (건물명)" 형태로 정상 유지, `source`="standardized"(좌표 확정 상태) 확인.
  3. **Functions ERROR 로그**: `firebase functions:log`로 `processReceipt` 최근 로그 조회 — **ERROR 0건**.
  4. **정리**: `processReceipt`는 코드상 Vision/Gemini/Kakao 조회 후 응답만 반환하는 순수 읽기 전용 함수(Firebase 쓰기 코드 없음)임을 코드로 재확인. 실제로 학습주소(`settings/learnedLocations`) 신규 생성이 없었음을 재조회로 확인(`null`), `orders`도 이번 검증으로 추가된 항목이 없음(해당 테넌트의 기존 실제 운영 데이터만 존재) — 애초에 삭제할 테스트 데이터 자체가 발생하지 않음.
...
## 2026-07-13 — OCR 실패 후 수기 입력 흐름 개선 배포 검증

- **대상**: `saas/driver.html` — `startOCR()`(`catch` 분기 라우팅, `base64` 스코프 확장, 에러에 `status` 부착), `confirmAdd()`(좌표 보강 + geocode 실패 시 저장 차단, `_pendingOCR` 보존 순서 조정)
- **커밋**: `efbb2cc`(핵심 구현), `a1a4de8`(APP_VERSION 17 범프)
- **배포 전 확인**: `git status` clean(`saas/driver.html` 1개 파일만 스코프, `functions/`·`database.rules.json`·`firebase.json`·`storage.rules` 변경 없음), `node --test "test/*.test.js"` **142/142 pass**(functions 미변경). driver.html 인라인 스크립트를 정규식으로 추출해 `new Function()`으로 구문 검사 — 에러 없음.
- **배포**: `firebase deploy --only hosting --project hatdelivery-saas`만 실행. `firebase database:set /settings/appVersion --data '"17"'`로 배포 직후 갱신(재조회로 `"17"` 확인).
- **배포 후 라이브 검증(wamartmillak 테스트용 기사 계정, Playwright로 라이브 도메인에서 `window.fetch`를 `processReceipt`/`geocodeAddress` 요청에만 국한해 모킹 — 나머지 요청은 원본 `fetch`로 통과시켜 실제 Firebase Auth/RTDB는 그대로 사용, 합성 데이터만 사용)**:
  1. **Vision/Gemini/500 실패 → result 화면 진입**: `processReceipt`가 500을 반환하도록 모킹 후 캔버스로 생성한 합성 이미지로 실제 `startOCR()` 호출 → `window._pendingOCR.source==='raw_fallback'`, `confidence==='low'`, `receiptDataUrl` 존재, `lat`/`lng` 둘 다 `null` 확인. 결과 화면 상태바 "🔴 주소 입력 필요", 입력 필드(이름/주소) 빈 값 확인.
  2. **401/403 → 기존 alert 유지**: `processReceipt`가 401을 반환하도록 모킹 후 호출 → `alert()`로 기존 문구("로그인이 만료되었거나 인증에 실패했습니다. 다시 로그인해주세요.")가 그대로 표시됨을 다이얼로그 핸들러로 확인, 수락 후 `goTab('scan')`(스캔 탭)으로 이동, `window._pendingOCR`은 `null`로 유지됨(변경 없음) 확인.
  3. **사진 썸네일 표시**: 1번 상태에서 결과 화면에 영수증 사진 썸네일(`img`)이 정상 렌더링됨을 스냅샷으로 확인.
  4. **사진 크게 보기**: "🔍 사진 크게 보기" 클릭 → `#photo-viewer` 모달에 방금 촬영한 합성 이미지가 그대로 표시됨을 스크린샷으로 확인.
  5. **다시 촬영 링크**: "📷 사진이 잘 안 나왔다면 다시 촬영하기" 클릭 → 스캔 탭(카메라 촬영 화면)으로 정상 이동 확인.
  6. **직접 입력 후 geocodeAddress 호출 및 좌표 포함 저장**: 1번 상태에서 이름/주소(마트 인근 공개 도로명)를 입력하고 "📍 주소 입력" 버튼 클릭 → 네트워크 모킹을 걸지 않은 실제 `geocodeAddress` 엔드포인트가 호출되고, 응답 좌표로 배송이 저장됨을 `deliveries` 배열 직접 조회로 확인(`lat`/`lng` 정상 값).
  7. **geocode 실패 시 저장 차단**: `geocodeAddress`가 실패를 반환하도록 모킹한 뒤, 존재하지 않는 가짜 주소로 재현 → 저장이 진행되지 않고(신규 배송 미생성) 토스트 "주소를 찾을 수 없습니다. 주소를 다시 확인해주세요." 노출 확인. **이 시점에 `window._pendingOCR`이 여전히 살아있어(사진 포함) 재시도가 가능함을 별도로 확인**(설계 단계에서 발견한 `_pendingOCR` 조기 초기화 문제의 수정이 실제로 동작함을 실측).
  8. **상세주소/출입정보/결제수단 저장**: 7번의 실패 상태에서 주소를 올바른 값으로 정정하고 상세주소/출입정보를 입력, 결제방식(현장결제)을 선택 후 재시도 → 이번엔 정상 저장되며 `detailAddress`/`accessInfo`/`paymentMethod` 전부 정확히 저장됨을 확인(6번의 재시도가 성공하는 케이스이기도 함).
  9. **기존 정상 OCR 흐름 회귀 없음**: `processReceipt`가 성공 응답(주소/상세주소/합계금액 포함)을 반환하도록 모킹 후 호출 → 결과 화면에 주소/상세주소/합계금액이 기존과 동일하게 정상 파싱·표시됨을 확인(이번 변경이 `startOCR()`의 성공 경로 자체는 건드리지 않았음을 재확인).
  10. **모바일 375px**: 375×812 리사이즈 후 홈 화면과 실패 결과 화면 둘 다 스크린샷으로 필드/버튼/네비게이션 겹침 없음을 확인.
  11. **콘솔 런타임 에러 없음**: 전체 콘솔 에러 조회 — 의도적으로 유발한 `console.error('OCR 오류:', ...)` 3건(1/2/10번 검증 과정에서 발생) 외 예외/런타임 에러 없음.
  12. **합성 데이터 삭제**: 검증 중 생성된 합성 배송 2건을 `tref('orders/'+id).remove()`로 삭제 후 재조회로 `null` 확인, `settings/learnedLocations`에 신규 레코드 없음 확인(이번 변경의 좌표 보강 분기는 학습 저장을 호출하지 않음), Storage에 영수증 사진이 업로드되지 않았음을 `getDownloadURL()` 재조회 실패(`storage/object-not-found`)로 확인.
...
## 2026-07-14 — 배송카드 compact layout 1~3차 배포 검증

- **대상**: `saas/driver.html` — `_renderDeliveryCard()`/`_renderCardInfoBadges()`와 관련 CSS(`.dl-info-line`, `.dl-badge-line`/`.dl-badge-slim`, `.dl-right`, `.dl-row-btns`/`.dl-btn-sm`/`.dl-btn-text-sm` 등)
- **커밋**: `267f8a2`(1차), `066da78`(2차), `743d5e1`(3차), `8f734ff`(APP_VERSION 21 범프, 1~3차 일괄 배포)
- **배포 전 확인**: `git status` clean(`saas/driver.html` 1개 파일만 스코프, `functions/`·`database.rules.json`·`firebase.json`·`storage.rules` 변경 없음, 세션 시작부터 있던 무관한 `.docx` 파일은 스테이징에서 제외), `node --test "test/*.test.js"` **142/142 pass**(functions 미변경, 세 라운드 각각 재확인).
- **구현 단계 실측 검증(매 라운드, 배포 전 로컬 정적 서버 2개 동시 구동 — 직전 라운드 커밋본과 현재 작업본을 각각 별도 포트로 서빙해 동일한 합성 데이터로 카드 높이 직접 비교, 재구현이 아닌 실제 코드 실행 비교)**: 모든 필드/최소 정보/사진+금액+배지/나중에 배송/좌표 없음/긴 출입정보/긴 이름+배지 다중조합 극단 케이스 7종 전부에서 3차 최종본이 이전 라운드와 같거나 작은 높이임을 확인. 이 과정에서 두 가지 CSS 버그를 자체 발견해 수정(상세 내역은 `_bmad/backlog.md`의 "TMS 배송카드 compact layout 1~3차" 항목 참고).
- **배포**: `firebase deploy --only hosting --project hatdelivery-saas`만 실행. `firebase database:set /settings/appVersion --data '"21"'`로 배포 직후 갱신(재조회로 `"21"` 확인). `curl`로 라이브 소스 조회해 `const APP_VERSION = '21'` 반영 확인.
- **배포 후 라이브 검증(wamartmillak 테스트용 기사 계정, Playwright로 라이브 도메인 직접 조작 — 합성 데이터는 대부분 localStorage 전용으로 시드해 Firebase 쓰기 최소화, 사진 첨부 케이스만 실제 저장)**:
  1. **로그인/버전 확인**: 라이브 접속 시 세션 자동 복원, `APP_VERSION==='21'` 확인.
  2. **배송중 카드 compact 3차 레이아웃**: 스크린샷으로 여백/구조 정상 반영 확인.
  3. **나중에 배송 섹션**: "⏭ 나중에 배송 (1)" 섹션 + "복귀" 텍스트 버튼 정상 노출 확인.
  4. **완료 탭 카드**: 길찾기/되돌리기/수정 3버튼(나중/복귀 없음), 배지 슬림 줄 정상 표시 확인.
  5. **상세주소/출입정보 전체 폭 표시**: `.dl-info-line`이 카드 전체 폭을 사용함을 확인.
  6. **긴 출입정보 줄바꿈**: 스크린샷으로 자연스러운 줄바꿈(끊기지 않음) 확인.
  7. **학습/사진 배지 의미 표시**: "📚학습"(`title="학습주소 적용"`), "🖼사진"(`title="이전 배송사진 있음"`) 텍스트+아이콘 확인.
  8. **나중/복귀 버튼 텍스트**: 아이콘 단독이 아닌 "나중"/"복귀" 텍스트로 표시됨을 확인.
  9. **길찾기/완료 버튼 크기 유지**: 버튼 4개("길찾기"/"완료"/"나중"/"✏️") 전부 잘림·줄바꿈 없이 표시됨을 확인.
  10. **ETA/전화/금액 우측 표시**: 우측 컬럼에 정상 배치 확인.
  11. **이전사진 썸네일 카드**: 실제 `addManual()`+`savePhoto()`로 생성한 카드에서 썸네일+배지 정상 확인.
  12. **모바일 375px**: `document.body.scrollWidth === window.innerWidth === 375` — 가로 스크롤 없음, 스크린샷으로 겹침 없음 확인.
  13. **버튼 클릭 회귀**: 실제 DOM 클릭으로 나중↔복귀 토글, 수정 모달 오픈, 완료화면 진입 전부 에러 없이 확인.
  14. **콘솔 에러**: 기존에 알려진 `kakaoWaypoints` 403(경로최적화 진입 시 발생하는 무관한 기존 이슈, `_bmad/backlog.md`에 이미 기록됨) 외 신규 에러 없음.
  15. **합성 데이터 삭제**: 사진 첨부 검증용으로 실제 생성된 order 1건 + IndexedDB 사진 1건을 삭제 및 재조회로 `null` 확인, 나머지는 localStorage 전용이라 정리 대상 없음(재확인 완료).
- **Result**: compact layout 1~3차가 실제 배포본에서 설계·구현 그대로 동작하고, 카드 높이·정보 배치·버튼 크기가 실측 목표(2차 대비 회귀 없음, 특히 사진+금액+배지 조합/긴 이름+다중배지 극단 케이스)를 실제 라이브 환경에서도 만족함을 확인. 기존 클릭 동작 회귀 없음, 신규 콘솔 에러 없음.
- **Sensitive data policy**: 합성 데이터만 사용(가짜 이름 "라이브검증A~D" 패턴, 가짜 전화번호). 검증에 사용한 도로명은 실존하는 공개 지명이지만 특정 개인과 무관. 검증 중 실제 생성된 order 1건 + IndexedDB 사진 1건 삭제 및 재조회로 잔존 없음(`null`) 확인. wamartmillak 테스트용 기사 계정 비밀번호는 이 로그/커밋/대화 응답 어디에도 기록하지 않음.

## 2026-07-14 — 건물명 보강 detailAddress compact layout 유지 검증

- **대상**: `saas/driver.html`(배송카드/경로최적화 리스트/완료화면의 `detailAddress` 표시 경로) — 이전 라운드에서 별도로 완료된 `functions/lib/receipt-utils.js`의 `enrichDetailAddressWithBuildingName()` 자체는 변경 없음, 이번 compact layout 변경(1~3차)이 그 표시 결과에 영향을 주지 않는지만 재확인.
- **배경**: compact layout 1~3차가 `_renderDeliveryCard()`의 상세주소 표시 구조(`.dl-info-line`)를 두 차례 재배치했기 때문에, 별도로 이미 완료돼 있던 "아파트/빌라명 상세주소 자동 보강" 기능의 표시 결과("동 호수 (건물명)" 형태)가 계속 그대로 유지되는지 실측으로 교차 확인.
- **추가 배포**: 없음 — 기존 `APP_VERSION 21` 배포본(compact layout 1~3차와 함께 이미 배포됨)으로만 검증, Functions/Hosting 추가 배포 없음.
- **검증(wamartmillak 테스트용 기사 계정, 실제 `processReceipt` 엔드포인트를 합성 영수증으로 호출 — 합성 데이터만 사용)**:
  1. **실제 OCR 경로로 건물명 보강 유도**: 합성 영수증(가짜 이름/가짜 전화번호, 주소는 "공개 아파트명 + 임의 동/호수" 패턴 — 실존하는 카카오맵 등록 공개 아파트명이지만 특정 개인과 무관)을 캔버스로 렌더링해 실제 `processReceipt` 엔드포인트로 호출.
  2. **응답 형식 확인**: 응답 `location.detail_address`가 "동 호수 (공개 아파트명)" 형태로 정확히 보강됨을 확인. 같은 호출의 `candidates` 1순위도 정답 아파트로 정렬됨을 확인해, 이전 라운드에서 배포한 Kakao 채점식 개선이 이번 변경과 무관하게 여전히 유효함을 함께 재확인.
  3. **배송목록 추가**: 실제 `confirmAdd()`로 Firebase `orders/{id}`에 저장, `detailAddress` 필드가 보강된 형태 그대로 저장됨을 확인.
  4. **배송카드 compact layout 표시**: `.dl-info-line`에 보강된 문자열이 줄바꿈 없이 그대로 표시됨을 DOM 조회 및 스크린샷으로 확인.
  5. **경로최적화 목록 / 완료화면**: 실제 `optimizeRoute()`/`openCompleteScreen()` 호출 후 두 화면 모두 동일한 보강 문자열이 그대로 표시됨을 확인.
  6. **수기 입력 경로는 자동 보강 대상 아님(정상 동작 확인)**: 실제 `addManual()`로 동일한 "동/호수" 텍스트를 상세주소 입력칸에 직접 입력 → 저장된 `detailAddress`에 건물명이 자동으로 붙지 않고 입력한 값 그대로 저장됨을 확인. `geocodeAndAdd()` 경로는 애초에 건물명 보강 로직을 거치지 않는 기존 서버 코드 구조와 일치하는 정상 동작.
  7. **콘솔 에러**: 기존에 알려진 `kakaoWaypoints` 403(경로최적화 화면 진입 시 발생, 이번 검증과 무관한 기존 이슈) 외 신규 에러 없음.
  8. **합성 데이터 삭제**: 건물명 보강 검증용 order 1건 + 수기입력 대비검증용 order 1건, 총 2건을 `tref('orders/'+id).remove()`로 삭제 후 재조회로 둘 다 `null` 확인.
- **Result**: compact layout 1~3차 변경이 별도로 완료돼 있던 건물명 자동 보강 기능의 실제 표시 결과에 영향을 주지 않음을 확인 — 배송카드/경로최적화/완료화면 3곳 모두 "동 호수 (건물명)" 형태가 그대로 유지됨. 수기 입력 경로는 설계대로 자동 보강 대상이 아님을 재확인.
- **Sensitive data policy**: 합성 데이터만 사용(가짜 이름 "검증고객건물"/"수기입력검증" 패턴, 가짜 전화번호). 검증에 사용한 아파트명은 카카오맵에 등록된 공개 장소 정보이며 특정 개인과 무관 — 본 로그에는 "공개 아파트명"으로만 표기. 검증 중 생성된 order 2건 전부 삭제 및 재조회로 잔존 없음(`null`) 확인. wamartmillak 테스트용 기사 계정 비밀번호는 이 로그/커밋/대화 응답 어디에도 기록하지 않음.

## 2026-07-14 — OMS: 좌표 없는 주문 자동배정 가시성 개선 1차 배포 검증

- **대상**: `saas/app.html`만 — `_countUnassignedNoCoord()`(신규 공용 헬퍼), `toggleAutoAssignBar()`, `previewAutoAssign()`, `runAutoAssign()`, `renderOrderList()`(`oc-bottom` 배지)
- **커밋**: `3e49a15`
- **배경**: read-only TMS/OMS 안정화 점검 라운드에서 자동배정이 좌표 없는 미배정 주문을 조용히 제외하면서도 운영자에게 이를 알리는 표시가 전혀 없음을 확인 — 배송 누락 위험으로 판단해 백로그 등록 없이 바로 처리.
- **배포 전 확인**: `git status` clean(`saas/app.html` 1개 파일만 스코프, `functions/`·`database.rules.json`·`firebase.json`·`storage.rules`·`saas/driver.html` 미변경, 세션 시작부터 있던 무관한 `.docx` 파일은 스테이징에서 제외), `node --test "test/*.test.js"` **142/142 pass**(functions 미변경), app.html 인라인 스크립트 구문 검사(정규식 추출 + `new Function`) 에러 없음.
- **배포 전 합성 데이터 로직 검증**: 실제 수정된 함수 본문을 그대로 옮긴 Node 샌드박스에서 합성 주문 5건(좌표 있는 근거리 미배정 1건, 좌표 없는 미배정 2건, 좌표 있는 장거리 미배정 1건, 배정완료 1건)으로 **18개 체크 전부 PASS** — 상단 요약/미리보기/실행결과 메시지에 좌표 없음 제외 카운트 정상 표시, `assignDriver()`가 좌표 있는 근거리 주문에만 호출(좌표 없는 주문은 배정 대상에서 계속 제외), 주문 카드 배지가 좌표 없는 주문에만 표시(좌표 있는 근거리/장거리/배정완료 주문에는 미표시), 기존 미배정/장거리 분류(좌표 기준) 회귀 없음.
- **배포**: `firebase deploy --only hosting --project hatdelivery-saas`만 실행. Functions/`database.rules.json`/`firebase.json`/`storage.rules` 미배포, `saas/driver.html`의 `APP_VERSION`/`settings/appVersion`도 변경 없음(OMS `app.html`은 이번 세션에 처음 배포된 파일이라 TMS 같은 버전 동기화 메커니즘 자체가 없음).
- **배포 후 정적 검증**: 라이브 URL(`hatdelivery-saas.web.app/saas/app.html`)에서 소스를 직접 내려받아 커밋(`3e49a15`)과 바이트 단위 비교 — 줄바꿈 문자(CRLF/LF) 차이만 있고 실제 내용은 100% 일치 확인. 좌표 없음 관련 코드 5곳(`_countUnassignedNoCoord`/`toggleAutoAssignBar`/`previewAutoAssign`/`runAutoAssign`/배지) 전부 라이브 소스에 반영됨을 grep으로 확인.
- **실제 화면(라이브 로그인) 검증**: **미실시**. 이 세션에 OMS(Firebase Auth 이메일/비밀번호) 로그인 계정이 없었음. 사용자가 로그인 계정 정보를 채팅으로 여러 차례 직접 전달했으나, 비밀번호를 로그인 필드에 입력하는 행위는 운영 규칙상 예외 없이 금지되어 있어 사용자가 명시적으로 제공/승인한 경우에도 대리 로그인을 진행하지 않고, 사용자가 직접 로그인해 확인하거나 정적 검증 수준에서 마무리하는 두 가지 선택지를 안내함 — 사용자가 후자를 선택.
- **Result**: "정적 배포 검증 완료, 실제 OMS 화면 검증은 후속 확인 필요"로 정리. 라이브 소스=커밋 소스 100% 일치 + 배포 전 합성 로직 검증 18/18 PASS로 배포된 코드 자체의 정확성은 확인됐으나, 실제 브라우저 화면에서 상단 요약/미리보기 메시지/실행결과 메시지/주문 리스트 배지의 육안 표시 확인(3~7번 항목)은 아직 남아있음.
- **후속(별도 확인 필요)**: OMS 계정으로 로그인 가능한 세션에서 (1) 상단 자동배정 요약 좌표없음 카운트, (2) 자동배정 미리보기 메시지, (3) 자동배정 실행 결과 메시지, (4) 메인 주문 리스트 배지, (5) 좌표 있는/장거리/배정완료 주문에 오표시 없는지, (6) 콘솔 에러 없는지 실제 화면에서 확인 필요. `_bmad/backlog.md`의 "진행 대기 (정리/품질)" 섹션에 동일 항목 등록.
- **Sensitive data policy**: 코드 변경/테스트/정적 검증 전 과정에서 합성 데이터만 사용, 실제 고객정보 미접촉. 라이브 화면 로그인 자체를 시도하지 않았으므로 이번 라운드에서 Firebase에 생성된 합성 데이터는 없음(정리 대상 없음). 사용자가 채팅으로 전달한 로그인 계정 정보(이메일/비밀번호)는 본 로그/backlog.md/커밋 메시지 어디에도 기록하지 않았으며, 해당 정보로 실제 로그인을 시도하지도 않음.

## 2026-07-14 — TMS 배송목록 드래그 정렬 + ETA/routeOrder 동기화 배포 검증

- **대상**: `saas/driver.html`만 — `_initDeliverySortable()`/`_reorderSectionFromDom()`/`_destroyDeliverySortables()`(1차, 드래그 정렬), `_syncRouteOrderFromDeliveries()`/`renderHome()`의 routeOrder 자동 채움(2차, ETA/경로최적화 동기화), `postponeDelivery()`/`unpostponeDelivery()`(버그 수정, routeOrder 통째 재조립으로 통일)
- **커밋**: `de8a498`(1차 드래그 정렬), `01c55eb`(APP_VERSION 22), `ce86c23`(2차 routeOrder/ETA 동기화), `37fba48`(APP_VERSION 23), `fb3d559`(routeOrder 부분 조작 버그 수정), `78ecd72`(APP_VERSION 24)
- **배경**: "TMS 배송목록 중심 순서 조정 UX" read-only 분석 라운드에서 나온 1차 MVP를 구현 후, 배포된 라이브에서 사용자가 직접 "배송목록 순서를 바꿔도 ETA/경로최적화 화면에 반영이 안 된다"는 문제를 발견해 2차로 확장. 2차 배포 직후 실측 검증 중 "드래그 후 나중→복귀 왕복 시 ETA가 부정확하게 튀는" 버그를 자체 발견해 즉시 수정한 3단계 라운드.
- **1차(드래그 정렬) 배포 전 확인**: `git status` clean(`saas/driver.html` 1개 파일만 스코프), `node --test "test/*.test.js"` **142/142 pass**, 인라인 스크립트 구문 검사 에러 없음. 배포 전 로컬 정적 서버 2개(직전 라운드본/작업본) 비교 없이, 실제 커밋된 `onEnd` 콜백을 DOM 조작 후 직접 호출하는 방식으로 저장/렌더링 로직 검증 — 배송중 3건 드래그, 나중에 배송 2건 드래그, 섹션 간 미섞임(`Sortable` `group` 옵션 `name:null` 확인), 새로고침 후 순서 유지, 완료 처리 후 순번 재계산, 나중/복귀 버튼 회귀 없음, 경로최적화 화면 회귀 없음, 모바일 375px 겹침 없음 전부 확인.
- **1차 배포**: `firebase deploy --only hosting --project hatdelivery-saas`. `APP_VERSION` 21→22 범프, `settings/appVersion` "22" 갱신, 라이브 소스 바이트 단위 커밋과 100% 일치 확인.
- **1차 라이브 검증**: 로그인 화면을 JS로 우회(비밀번호 미입력, Firebase 인증 없음) + 합성 localStorage 데이터로 위 항목 전부 재확인. 실제 마우스/터치 드래그 제스처는 `computer` 도구(스크린샷/좌표 드래그) 타임아웃 + 합성 `MouseEvent`/`PointerEvent` dispatch로도 SortableJS가 트리거되지 않아 **미검증** — 사용자가 실기기에서 직접 확인해 "순번 원 잡고 정상 동작" 확인.
- **2차(routeOrder/ETA 동기화) 배경**: 사용자가 라이브에서 직접 드래그해보고 "카드 순서만 바뀌고 도착 예상 시간/경로최적화 화면은 그대로"라고 피드백. 기사 UX 관점에서 "배송목록 순서 변경 = 이 순서로 배송하겠다는 의도"이므로 ETA/경로최적화에도 반영돼야 한다는 원칙에 합의.
- **2차 구현/검증(1차 설계, A안 → 사용자 피드백으로 B안 전환)**: `_syncRouteOrderFromDeliveries()`가 배송중 항목을 나중에 배송 항목보다 항상 앞에 두도록 `routeOrder` 재조립, `_routeCache.key`/`timeBannerText`도 함께 갱신. 최초 구현(A안, 드래그 시에만 sync)에서는 새로고침 시 `routeOrder`가 메모리 전용이라 빈 배열로 리셋되며 ETA가 standalone으로 되돌아가는 갭을 자체 검증 중 발견해 보고 → 사용자가 "새로고침 후에도 카드 순서가 유지되면 ETA도 유지되는 게 맞다"는 기준으로 B안(routeOrder가 비어있으면 `renderHome()`이 매번 현재 deliveries 순서로 자동 채움) 지시 → B안으로 재구현. 로컬 정적 서버에서 드래그 없이도 첫 렌더부터 목록 순서 기준 ETA 계산, 드래그 후 재계산, 새로고침 후 순서+ETA 함께 유지, 경로최적화 화면에 수동 순서 반영(캐시 히트), 신규 주문 추가(구성 변경)로 캐시 미스 유도 시 자동 최근접이웃 재정렬 복귀, 나중에 배송 섹션 드래그가 배송중 ETA에 무영향 전부 확인.
- **2차 배포**: `APP_VERSION` 22→23 범프, `settings/appVersion` "23" 갱신, 라이브 소스 100% 일치 확인.
- **2차 라이브 검증 중 자체 발견한 버그**: 라이브에서 "드래그 후 나중→복귀 왕복" 시나리오를 검증하다가 특정 카드의 ETA가 04:51→05:06으로 부정확하게 튀는 것을 발견. 원인 추적: `postponeDelivery()`의 기존 `routeOrder` 부분 조작(대상 항목만 찾아 맨 뒤로 splice+push)이 드래그로 만든 "배송중 우선" `routeOrder` 순서를 깨뜨림 — 나중 처리된 항목이 복귀해도 여전히 나중에 배송 항목들보다 뒤에 남는 정합성 오류. 나중에 배송 섹션 단독 드래그(배송중에 무영향)는 격리 재검증으로 문제없음을 별도 확인해 원인을 정확히 좁힘.
- **버그 수정**: `postponeDelivery()`/`unpostponeDelivery()` 둘 다 `routeOrder` 부분 조작 제거, `saveDeliveries()` 직후 `_syncRouteOrderFromDeliveries()` 호출로 통일. `node --test` 142/142 재확인. 로컬 정적 서버에서 정확히 재현했던 버그 시나리오(드래그 후 나중→복귀 왕복)를 그대로 재실행해 `routeOrder`가 "배송중 우선" 원칙을 유지하고 ETA도 화면 순서와 정확히 일치하게 재계산됨을 확인, 드래그 없는 나중/복귀 단독 케이스·나중에 배송 섹션 무영향·경로최적화 반영/재실행·새로고침 유지·버튼 회귀 전부 재검증.
- **버그 수정 배포**: `APP_VERSION` 23→24 범프, `settings/appVersion` "24" 갱신, 라이브 소스 100% 일치 확인. 라이브에서 동일한 버그 재현 시나리오를 재실행해 수정 반영 확인(ETA가 화면 순서와 정확히 일치하게 증가, 재발 없음), 나머지 전체 체크리스트(1~12번) 라이브 재검증 — 모바일 375px 겹침 최초 오탐(경로최적화 화면에 남아있던 상태에서 측정한 아티팩트)을 홈 화면 재측정으로 정정, 실제 겹침/가로스크롤 없음 확인, 콘솔 에러 0건.
- **사용자 실기기 최종 확인**: 순번 원 드래그 후 ETA 재계산, 드래그 후 나중→복귀 왕복 시 ETA 정합성, 새로고침 후 순서/ETA 유지, 순번 원 터치 영역 크기 4가지 항목 전부 실기기에서 직접 확인 후 정상 확인.
- **Result**: 배송목록 드래그 정렬(1차) + routeOrder/ETA/경로최적화 화면 동기화(2차) + routeOrder 부분 조작 버그 수정, 3단계 전부 배포·검증 완료. 라이브 소스는 매 단계 커밋과 바이트 단위 100% 일치, `node --test` 142/142 유지, 실기기 확인까지 완료.
- **Sensitive data policy**: 전 과정 합성 데이터만 사용(가짜 이름/전화번호/가상 주소). 모든 검증이 `localStorage`/메모리 전용으로 이뤄져 Firebase 쓰기 자체가 없었음(매 라운드 `read_network_requests`로 `firebaseio` 요청 0건 확인), 검증 종료 후 `localStorage.removeItem()` + 재조회로 `null` 확인. 사용자가 세션 중 채팅으로 전달한 OMS 계정 정보(이번 기능과 무관)는 이번 라운드에서도 사용·기록하지 않음.
