...
### TMS 영수증 원본 확대/줌 기능 개선 (1차: 버튼 확대/축소 + 드래그)
- **Status**: ✅ Deployed / Verified (Hosting만 배포, Functions/`database.rules.json`/`storage.rules` 미변경)
- **Commits**: `d857aad`(핵심 구현), `7143ca0`(APP_VERSION 19 범프)
- **배경**: 기사 피드백 — OCR 결과 확인 시 영수증 원본을 크게 봐도 주소/연락처/금액 글씨가 작아 확인이 어려움. read-only 분석 라운드에서 `#photo-viewer`가 OCR 원본/이전 배송사진/완료사진 3곳에서 재사용되는 단일 모달이며 확대/이동 기능이 전혀 없다는 점, 그리고 앱 전역 viewport 메타(`maximum-scale=1.0, user-scalable=no`)로 브라우저 기본 핀치줌이 이미 막혀 있어 모달 내부 JS transform 방식이 유일한 구현 경로라는 점을 확인 후 설계.
- **핵심 변경** (`saas/driver.html`만):
  - 새 모달을 만들지 않고 기존 `#photo-viewer` 그대로 재사용. 좌상단에 확대(`+`)/축소(`−`)/원래대로(`⟲`) 버튼 3개 신규 추가(우상단 기존 닫기 버튼과 겹치지 않는 반대편 배치).
  - `_photoZoom{scale,tx,ty}` 상태 + `zoomPhotoBy()`(0.5 단위 증감, 1~3배 클램프, 1배 도달 시 이동값도 함께 초기화) 신규.
  - Pointer Events(`pointerdown/move/up`) 기반 드래그 — **scale이 1을 초과할 때만** 이동 허용(1배에서는 드래그해도 이미지가 움직이지 않음).
  - 더블탭(300ms 이내 연속 탭) 시 1배↔2배 토글.
  - `viewPhoto()`(완료사진)/`viewPhotoUrl()`(이전 배송사진·OCR 원본)/`closePhotoViewer()` 3곳 모두에서 `resetPhotoZoom()` 호출 — 닫을 때는 물론, **다른 사진을 새로 열 때도** 이전 사진의 확대/이동 상태가 남아있지 않도록 함(세 사용처가 같은 DOM을 공유하므로 교차 오염 방지가 핵심).
  - 삭제 버튼 노출 조건(`btn-delete-photo`의 `display`)과 `currentPhotoId` 로직은 전혀 손대지 않음 — 기존 그대로 OCR 원본/이전 배송사진은 숨김, 완료사진은 표시.
  - **핀치줌은 2차로 분리**(설계 단계에서 사용자 승인) — 버튼 확대 + 드래그만으로 핵심 불편(작은 글씨 확인)이 상당 부분 해소된다고 판단, 구현/검증 복잡도가 높은 멀티터치 핀치는 이번 범위에서 제외.
- **테스트**: `node --test "test/*.test.js"` **142/142 pass**(functions 미변경). 구현 단계에서는 로컬 정적 서버로 실제 커밋 대상 코드를 서빙해 로그인 없이 `viewOcrPhoto()`/`viewPhotoUrl()`/`viewPhoto()`(`getPhoto` 스텁)를 직접 호출하고 `PointerEvent`/`KeyboardEvent`를 디스패치해 드래그·더블탭·ESC·배율 클램프를 전부 실측 검증.
- **Deploy**: `firebase deploy --only hosting --project hatdelivery-saas`. `DEPLOY_CHECKLIST.md` §4-1 절차대로 배포 전 `APP_VERSION` 18→19 범프, 배포 후 `settings/appVersion`도 "19"로 갱신.
- **배포 후 라이브 검증**: `_bmad/tea/evidence-log.md`의 "2026-07-13 — TMS 영수증 원본 확대/줌 기능 배포 검증" 참고. wamartmillak 테스트용 기사 계정으로 OCR 결과화면/이전 배송사진/완료사진(실제 `addManual()` + IndexedDB 사진 첨부로 재현) 3개 경로 전부에서 확대/축소/원래대로/드래그/더블탭/배율 클램프(1~3배)/닫기·ESC·사진 전환 시 초기화/삭제 버튼 노출 조건을 실제 `PointerEvent`/`KeyboardEvent` 디스패치로 확인, 모바일 375px 컨트롤 겹침 없음, 신규 콘솔 에러 없음 전부 확인.
- **후속(이번 범위 밖)**: 핀치줌(멀티터치), 확대 중 관성 스크롤/바운스, 확대 배율 표시 UI, 이미지 회전.
- **Sensitive data policy**: 합성 데이터만 사용(가짜 이름/가짜 전화번호). 검증 중 실제 `addManual()`+`savePhoto()`로 생성된 합성 배송 1건(Firebase orders + IndexedDB 사진)을 전부 삭제 및 재조회로 잔존 없음(`null`) 확인. wamartmillak 테스트용 기사 계정 비밀번호는 이 문서/커밋/로그 어디에도 기록하지 않음.

### TMS 배송카드 compact layout 1~3차
- **Status**: ✅ Deployed / Verified (Hosting만 배포, Functions/`database.rules.json`/`firebase.json`/`storage.rules` 미변경)
- **Commits**: `267f8a2`(1차: 상세주소+출입정보 병합/우측 컬럼 활용/여백 축소), `066da78`(2차: 반복배송 배지 아이콘화/보조버튼 축소), `743d5e1`(3차: 배지·상세정보 전체 폭 재배치, 나중/복귀 텍스트 복원), `8f734ff`(APP_VERSION 21 범프, 1~3차 일괄 배포)
- **배경**: 기사 피드백 — 배송목록 카드가 크고 정보가 왼쪽에 몰려 한 화면에 보이는 배송 수가 적음. 3라운드에 걸쳐 read-only 분석 → 구현 → 실측 검증을 반복하며 카드 밀도를 단계적으로 개선.
- **핵심 변경** (`saas/driver.html`만, `_renderDeliveryCard()`/`_renderCardInfoBadges()`와 관련 CSS 중심):
  - **1차**: 상세주소+출입정보를 한 줄로 병합, 중복 배지("🔐 출입정보 있음") 제거, 전화/금액을 우측 컬럼으로 이동, 카드 여백 축소.
  - **2차**: 반복배송 배지(학습주소/이전사진)를 텍스트 pill에서 아이콘 전용으로 축소, 나중/복귀/수정을 소형 아이콘 버튼으로 축소(길찾기/완료는 크기 유지).
  - **3차**: "고객명 옆" 배지 배치가 사진 썸네일이 있는 카드에서 좌측 컬럼 폭 부족(실측 93px)으로 줄바꿈을 유발해 카드가 오히려 커지는 회귀를 실측으로 발견 → 배지를 카드 전체 폭을 쓰는 slim 줄로 재배치(`.dl-info-line`과 동일한 정렬 방식). 아이콘 단독은 의미가 약하다는 피드백으로 "나중"/"복귀" 버튼 텍스트를 복원(단, 소형 auto-width 버튼 유지). 상세주소/출입정보도 계속 카드 전체 폭 사용.
  - **구현 중 자체 발견·수정한 두 가지 CSS 버그**: (1) 2차에서 아이콘 버튼 font-size를 크게 잡아 flex `stretch` 특성상 오히려 이 버튼이 행에서 가장 큰 요소가 되어 버튼 padding 축소 효과가 상쇄되는 문제, (2) 3차에서 텍스트 버튼에 `flex-basis:auto`만 주고 `width`를 안 정해줘 기본 `.btn{width:100%}`가 상속되며 flex-basis가 100%로 풀려 길찾기/완료가 거의 0폭으로 짜부라지는 문제 — 둘 다 실측으로 발견해 수정.
  - OCR 결과화면의 `.info-badge-pill`(`renderResultBadges()`)은 배송카드 전용 클래스(`.dl-badge-slim` 등)와 완전히 분리해 영향 없음. 완료/사진/지도/경로최적화/geocode/save 로직은 전혀 변경하지 않음.
- **테스트**: `node --test "test/*.test.js"` **142/142 pass**(매 라운드 재확인, functions 미변경). 매 라운드마다 `git show HEAD:saas/driver.html`로 이전 라운드 버전을 별도 로컬 서버로 동시에 띄워 동일한 합성 데이터(모든 필드/최소 정보/사진+금액+배지/나중에 배송/좌표 없음/긴 출입정보/긴 이름+배지 다중조합 극단 케이스)로 카드 높이를 실측 비교 — 3차 최종 기준 7개 유형 전부 이전 라운드와 같거나 작음, 회귀 없음 확인.
- **Deploy**: `firebase deploy --only hosting --project hatdelivery-saas`. `DEPLOY_CHECKLIST.md` §4-1 절차대로 배포 전 `APP_VERSION` 20→21 범프(1차 배포 시점에 19→20을 이미 사용했고, 2·3차는 배포 없이 커밋만 누적됐던 상태라 이번에 1~3차를 한번에 배포하며 21로 조정), 배포 후 `settings/appVersion`도 "21"로 갱신.
- **배포 후 라이브 검증**: `_bmad/tea/evidence-log.md`의 "2026-07-14 — 배송카드 compact layout 1~3차 배포 검증"과 "2026-07-14 — 건물명 보강 detailAddress compact layout 유지 검증" 참고. wamartmillak 테스트용 기사 계정으로 배송중/나중에 배송/완료 탭 전부, 상세주소·출입정보 전체 폭 표시, 배지 텍스트("📚학습"/"🖼사진") 의미 표시, 나중/복귀 텍스트 버튼, 길찾기/완료 버튼 크기 유지, 이전사진 썸네일 카드, 모바일 375px 겹침/가로스크롤 없음, 기존 버튼 클릭 전부 회귀 없음을 확인. 추가로 아파트/빌라명 자동 보강 기능(별도 이전 완료 항목)이 이번 레이아웃 변경과 무관하게 배송카드/경로최적화/완료화면에서 그대로 유지됨을 실측 재확인.
- **참고(이번 라운드와 무관)**: 라이브 검증 중 `kakaoWaypoints` 403 콘솔 에러가 재확인됐으나, 이는 기존에 이미 기록된 무관한 이슈(위 "배송지도(mapzone) 경로선 기능 대체 설계 필요" 백로그 항목 참고).
- **후속(이번 범위 밖)**: 카드 펼침/접힘(C안) 등 더 큰 상호작용 변경, 버튼을 더보기(⋯) 메뉴로 축소하는 안(상호작용 변경 폭이 커 별도 승인 필요 판단), 배송목록+경로최적화 통합(장기 항목, 아래 별도 기록).
- **Sensitive data policy**: 합성 데이터만 사용(가짜 이름/가짜 전화번호). 건물명 보강 검증에 사용한 아파트명은 카카오맵에 등록된 공개 장소 정보(특정 개인과 무관)이며 본 문서에는 "공개 아파트명"으로만 표기. 검증 중 생성된 합성 배송 데이터 전부 삭제 및 재조회로 잔존 없음(`null`) 확인. wamartmillak 테스트용 기사 계정 비밀번호는 이 문서/커밋/로그 어디에도 기록하지 않음.

### OMS: 좌표 없는 주문 자동배정 가시성 개선 1차
- **Status**: ✅ Hosting 배포 / 정적 검증 완료 — ⚠️ **실제 화면 육안 검증은 후속 필요** (아래 참고)
- **Commit**: `3e49a15`
- **배경**: read-only 안정화 점검(TMS/OMS) 라운드에서 자동배정이 좌표 없는 미배정 주문을 조용히 제외하면서도 이를 운영자에게 알리는 표시가 전혀 없어 배송 누락 위험이 있음을 확인. 배송 누락 리스크가 있다고 판단해 백로그 등록 없이 바로 처리.
- **범위**: `saas/app.html`만 변경. 자동배정 알고리즘 구조/필터 조건은 변경하지 않고, 좌표 없는 주문을 배정 대상에 넣지도 않음 — 순수 가시성 개선.
- **핵심 변경**: 공용 헬퍼 `_countUnassignedNoCoord()` 신규(미배정 상태 중 lat/lng 없는 건수, 3곳에서 동일 기준 재사용) → (1) `toggleAutoAssignBar()`의 상단 요약(`auto-assign-desc`)에 "· ⚠️ 좌표 없음 N건 제외" 추가, (2) `previewAutoAssign()` 미리보기 메시지를 "자동배정 대상 X건 / 장거리 Y건 / 좌표 없음 Z건 제외" 형식으로 확장, (3) `runAutoAssign()` 실행 결과 메시지(조기 리턴 + 최종 성공 메시지 양쪽)에 동일 카운트 추가, (4) `renderOrderList()`의 `oc-bottom` 영역에 "⚠️ 좌표 없음" 배지 신규(기존 `oc-addr`의 `📍` 장식 아이콘과는 분리).
- **테스트**: `node --test "test/*.test.js"` **142/142 pass**(functions 미변경). app.html 인라인 스크립트 구문 검사(정규식 추출 + `new Function`) 에러 없음. 배포 전, 실제 수정된 함수 본문을 그대로 옮긴 Node 샌드박스에서 합성 데이터 5건(좌표 있는 미배정/좌표 없는 미배정×2/장거리/배정완료)으로 **18개 체크 전부 PASS** — 좌표 없는 주문이 배정 대상에서 여전히 제외됨, 배지가 좌표 없는 주문에만 표시됨, 기존 미배정/장거리 분류 회귀 없음을 확인.
- **Deploy**: `firebase deploy --only hosting --project hatdelivery-saas`만 실행. Functions/`database.rules.json`/`firebase.json`/`storage.rules` 미변경, `saas/driver.html`(APP_VERSION)도 이번 변경과 무관해 그대로 둠 — OMS(`app.html`)에는 TMS의 `APP_VERSION`/`settings/appVersion` 같은 버전 동기화 메커니즘이 존재하지 않음(이번 세션이 이 파일을 처음 배포).
...
- **실제 화면 검증 미실시 사유**: 이 세션에 OMS(Firebase Auth 이메일/비밀번호) 로그인 계정이 없었음. 사용자가 슈퍼어드민 계정 정보를 채팅으로 직접 전달했으나, 비밀번호를 로그인 필드에 입력하는 행위는 운영 규칙상 예외 없이 금지되어 있어 사용자가 명시적으로 제공/승인한 경우에도 대리 로그인하지 않고 사용자에게 직접 로그인해 확인해달라고 안내함. 이번 라운드는 사용자 승인 하에 "정적 배포 검증 완료, 실제 OMS 화면 검증은 후속 확인 필요"로 마무리.
- **후속(별도 확인 필요, 아래 "진행 대기" 참고)**: OMS 계정 확보 시 실제 화면에서 (1) 상단 자동배정 요약 좌표없음 카운트, (2) 자동배정 미리보기 메시지, (3) 자동배정 실행 결과 메시지, (4) 메인 주문 리스트 배지, (5) 좌표 있는/장거리/배정완료 주문에 오표시 없는지, (6) 콘솔 에러 없는지 육안 확인 필요.
- **Sensitive data policy**: 코드 변경/테스트/정적 검증 전 과정에서 합성 데이터만 사용, 실제 고객정보 미접촉. 사용자가 채팅으로 전달한 로그인 계정정보(비밀번호 포함)는 본 문서/커밋/로그 어디에도 기록하지 않았으며, 해당 정보로 실제 로그인을 시도하지도 않음.

### TMS 배송목록 드래그 정렬 + ETA/routeOrder 동기화
- **Status**: ✅ Deployed / Verified (Hosting만 배포, Functions/`saas/app.html`/`database.rules.json`/`firebase.json`/`storage.rules` 미변경)
- **Commits**: `de8a498`(1차: 드래그 정렬 핸들/섹션 분리), `01c55eb`(APP_VERSION 22 범프), `ce86c23`(2차: routeOrder/ETA/경로최적화 화면 동기화), `37fba48`(APP_VERSION 23 범프), `fb3d559`(버그 수정: postpone/unpostpone routeOrder 부분 조작 제거), `78ecd72`(APP_VERSION 24 범프)
- **배경**: "TMS 배송목록 중심 순서 조정 UX" read-only 분석 라운드에서 기사가 지도보다 배송목록을 보고 배송하는 경향이 크다는 점, 배송목록에서 직접 순서를 조정하는 기능이 없다는 점을 확인. 검토한 후보(위/아래 버튼, 순서 편집 모드, 드래그, 경로최적화 통합) 중 사용자가 "배송목록에서 직접 드래그 정렬"을 1차 MVP로 선택.
- **1차 범위** (`saas/driver.html`만): 배송중/나중에 배송 섹션을 각각 독립 `Sortable.create()`로 구성, 핸들을 순번 원(`.dl-num` → `.dl-drag-handle`)으로 제한(새 버튼/아이콘 추가 없음, 길찾기/완료/나중/복귀/수정 버튼과 충돌 없음). 두 섹션 컨테이너(`#dl-active-list`/`#dl-postponed-list`)를 완전히 분리해 `group` 옵션 미설정 — SortableJS 기본 동작상 컨테이너 간 드래그 이동 자체가 구조적으로 불가능. 순서는 `localStorage`에만 저장(`_reorderSectionFromDom()`이 `syncDeliveriesOrderFromRoute()`와 동일한 안정정렬 기법으로 `deliveries` 배열 갱신), Firebase `sortIndex` 도입 없음.
- **2차 확장(실사용 피드백 반영)**: 배포 후 실제 라이브에서 사용자가 "배송목록에서 순서를 바꿔도 ETA/경로최적화 화면에 반영이 안 된다"는 문제를 직접 발견 — 기사 입장에서 배송목록 순서 변경은 "이 순서로 배송하겠다"는 의도이므로 ETA/경로최적화 화면에도 반영되는 게 자연스럽다는 원칙으로 확장 범위 합의. `_syncRouteOrderFromDeliveries()` 신규(배송중 항목이 `routeOrder`에서 항상 나중에 배송 항목보다 앞에 오도록 재조립, 최근접 이웃 탐색 등 자동배정 알고리즘 자체는 호출 안 함), 드래그 `onEnd`마다 호출. 처음엔 "드래그한 순간에만 route 모드 전환, 새로고침 시 ETA는 standalone으로 리셋"(A안)으로 구현했으나, 사용자가 "새로고침 후에도 카드 순서가 유지된다면 ETA도 그 순서 기준으로 유지되는 게 맞다"는 기준을 명확히 해 B안으로 전환 — `renderHome()`이 ETA 계산 전 `routeOrder`가 비어있으면(드래그 이력 없음/새로고침 직후) 현재 `deliveries` 순서로 자동 채움(이미 채워진 routeOrder는 덮어쓰지 않음). `_routeCache.key`도 함께 갱신해 경로최적화 화면 진입 시 캐시 히트로 수동 순서를 그대로 보여주되, 주문 구성(배정/완료 등)이 바뀌면 캐시 키가 달라져 기존 로직대로 자동 재정렬이 다시 적용됨(`optimizeRoute()` 자체는 미변경).
- **배포 후 라이브에서 자체 발견/수정한 버그**: 2차 배포 직후 실측 검증 중, 드래그로 순서를 바꾼 뒤 "나중"→"복귀"를 왕복하면 ETA가 부정확하게 튀는 것을 발견. 원인은 `postponeDelivery()`에 있던 기존(이전 라운드부터 있던) `routeOrder` 부분 조작 로직(대상 항목만 찾아 `routeOrder` 맨 뒤로 이동)이 새로 만든 "배송중이 나중에 배송보다 항상 앞" 원칙과 충돌 — 드래그 후 나중 처리 시 해당 항목이 나중에 배송 항목들보다도 더 뒤로 밀려 ETA가 왜곡됨. `postponeDelivery()`/`unpostponeDelivery()` 둘 다 부분 조작 대신 `_syncRouteOrderFromDeliveries()` 호출로 통일해 수정, 라이브 재검증으로 재발 없음 확인.
- **테스트**: `node --test "test/*.test.js"` **142/142 pass**(매 라운드 재확인, functions 미변경). 매 라운드 로컬 정적 서버 + 실제 커밋된 `onEnd` 콜백을 DOM 조작 후 직접 호출하는 방식(SortableJS 자체의 마우스/터치 이벤트 처리는 이미 경로최적화 화면에서 프로덕션 검증된 라이브러리라 별도 재검증 안 함)으로 저장/렌더링/ETA/`routeOrder` 로직 검증.
- **Deploy**: `firebase deploy --only hosting --project hatdelivery-saas`. `DEPLOY_CHECKLIST.md` §4-1 절차대로 매 배포마다 `APP_VERSION` 범프(21→22→23→24) + `settings/appVersion` 동기화.
- **배포 후 라이브 검증**: `_bmad/tea/evidence-log.md`의 "2026-07-14 — TMS 배송목록 드래그 정렬 + ETA/routeOrder 동기화 배포 검증" 참고. 매 라운드 라이브 소스=커밋 바이트 단위 100% 일치 확인, 로그인 화면 우회 + 합성 localStorage 데이터로 저장/ETA/`routeOrder`/경로최적화 화면 반영/버튼 회귀/모바일 375px/콘솔 에러 전부 확인. **실제 손가락/마우스 드래그 제스처는 이 세션의 도구(스크린샷/좌표 드래그) 한계로 직접 검증하지 못해 사용자가 실기기에서 직접 확인** — 순번 원 드래그 시작, 드래그 후 ETA 재계산, 나중→복귀 왕복 후 ETA 정합성, 새로고침 후 순서/ETA 유지, 터치 영역 크기 4가지 전부 정상 확인.
- **후속(이번 범위 밖)**: 배송목록에서의 순서 표시 문구("도착 예상")가 "최적 경로"처럼 오해될 소지가 있음 — 현재는 최적화 여부와 무관하게 "현재 목록 순서 기준 예상"일 뿐인데 문구가 이를 구분하지 않음(코드 주석으로만 기록, 문구 개선은 후속 과제). 다중기기 순서 동기화(`routeOrder`/`deliveries` 순서 모두 로컬 전용이라 기기 변경 시 초기화됨)는 read-only 분석 단계에서 이미 알려진 한계로 1차 범위 밖.
- **Sensitive data policy**: 코드 변경/테스트/라이브 검증 전 과정에서 합성 데이터만 사용(가짜 이름/전화번호/가상 주소). 라이브 검증은 전부 `localStorage`에서만 이뤄져 Firebase 쓰기 자체가 없었음(매 라운드 `read_network_requests`로 `firebaseio` 요청 0건 확인), 검증 종료 후 `localStorage.removeItem()`으로 정리 및 `null` 확인. 사용자가 채팅으로 전달한 OMS 계정 정보(이번 기능과 무관)는 이번 라운드에서도 사용/기록하지 않음.
...
- ~~**영수증 원본 확대/줌/이동(핀치줌·더블탭줌·드래그)**~~ → **1차 완료**: 위 "완료" 섹션의 "TMS 영수증 원본 확대/줌 기능 개선 (1차: 버튼 확대/축소 + 드래그)" 항목 참고 — 버튼 확대/축소/원래대로 + 드래그 + 더블탭줌 구현·배포·검증 완료. 핀치줌(멀티터치)만 2차로 남음.
- ~~**배송목록 카드 compact layout 전면 재설계**~~ → **1~3차 완료**: 위 "완료" 섹션의 "TMS 배송카드 compact layout 1~3차" 항목 참고 — 정보 재배치, 배지/버튼 축소, 상세정보 전체 폭 사용까지 3라운드에 걸쳐 구현·배포·검증 완료. 카드 펼침/접힘, 더보기 메뉴 등 더 큰 상호작용 변경은 후속(4차 이후)으로 남음.
...
- **OMS 좌표 없는 주문 가시성 개선 — 실제 화면 육안 검증 필요** — 위 "완료" 섹션의 "OMS: 좌표 없는 주문 자동배정 가시성 개선 1차" 항목 참고. Hosting 배포 및 소스 정적 검증(라이브=커밋 100% 일치)까지는 완료했으나, 이 세션에 OMS 로그인 계정이 없어 실제 화면에서의 육안 검증(상단 요약/미리보기/실행결과 메시지/배지 표시/오표시 여부/콘솔 에러)은 미실시. OMS 계정으로 로그인 가능한 세션에서 후속 확인 필요.
