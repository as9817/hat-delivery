...
- **2026-07-08 갱신**: "이전 배송완료 사진 보기" MVP(`07ed9b8`) 배포 시에도 이 이슈는 그대로 미해결로 남겨두고 진행함 — 신규 기능이 별도 Storage 업로드를 하지 않고 기존 `photoUrl`(위 문단의 다운로드 토큰 방식)을 그대로 재사용하는 구조라, 이번 배포로 노출 범위가 더 넓어지지는 않았음(신규 리스크 추가 없음). 다만 "이전 사진"을 조회해서 보여주는 화면이 새로 생겨 이 미해결 이슈를 실제로 체감하는 사용자 접점은 늘어난 상태 — 별도 해결 라운드의 우선순위 근거로 참고.
